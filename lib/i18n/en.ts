export const en = {
  // Common
  common: {
    learnMore: "Learn More",
    bookNow: "Book Now",
    contactUs: "Contact Us",
    viewAll: "View All",
    from: "from",
    to: "To",
    km: "km",
    min: "min",
    currency: "EUR",
  },

  // Navigation
  nav: {
    home: "Home",
    services: "Services",
    fareCalculator: "Fare Calculator",
    contact: "Contact",
    toggleMenu: "Toggle menu",
  },

  // Language
  language: {
    english: "English",
    greek: "Greek",
    selectLanguage: "Select language",
  },

  // Contact options
  contact: {
    phone: "Phone",
    whatsapp: "WhatsApp",
    viber: "Viber",
    email: "Email",
    chatNow: "Chat Now",
    callOnViber: "Call on Viber",
    bookViaWhatsApp: "Book via WhatsApp",
    openContactMenu: "Open contact menu",
    closeContactMenu: "Close contact menu",
    contactVia: "Contact via",
  },

  // Footer
  footer: {
    copyright: "Copyright",
  },

  // Home page
  home: {
    heroTitle: "Your Trusted Transfer",
    heroSubtitle:
      "Professional private driver service from Thessaloniki Airport (SKG) to any destination. Safe, comfortable, and always on time.",
    
    // Stats
    stats: {
      hotels: "Hotels",
      hotelsLabel: "Hotels transfers",
      available: "24/7",
      availableLabel: "Available",
      airport: "SKG",
      airportLabel: "Thessaloniki Airport",
      tours: "Halkidiki",
      toursLabel: "City Tours",
    },

    // Services section
    servicesSection: "What We Offer",
    servicesTitle: "Our Transfer Services",
    viewAllServices: "View All Services",
    
    services: {
      airportPickup: {
        title: "Airport Pickup",
        description:
          "We meet you at arrivals with a name sign. Flight tracking included so we are always on time, even if your flight is delayed.",
      },
      airportDropoff: {
        title: "Airport Drop-off",
        description:
          "Timely pickup from your hotel or any location in Thessaloniki. We make sure you get to the airport stress-free.",
      },
      cityTours: {
        title: "City Tours",
        description:
          "Explore Thessaloniki with a local driver. Visit the White Tower, Ano Poli, the waterfront, and more at your own pace.",
      },
      longDistance: {
        title: "Long Distance",
        description:
          "Transfers to Halkidiki, Kavala, Katerini, and other destinations. Comfortable rides with competitive fixed prices.",
      },
    },

    // Features section
    featuresTitle: "Why Travelers Choose Us",
    features: {
      safe: {
        title: "Safe & Professional",
        description:
          "Licensed, insured, and experienced driver for your peace of mind.",
      },
      fixedPrices: {
        title: "Fixed Prices",
        description:
          "No hidden fees or surprises. Know your fare before you book.",
      },
      punctual: {
        title: "Always Punctual",
        description:
          "We track your flight and adjust pickup time automatically.",
      },
      meetGreet: {
        title: "Meet & Greet",
        description: "Personal welcome at the airport with your name sign.",
      },
      multilingual: {
        title: "Multilingual",
        description:
          "Communication in Greek, English, Russian and more for your comfort.",
      },
      comfortable: {
        title: "Comfortable Vehicle",
        description:
          "Clean, air-conditioned sedan with space for your luggage.",
      },
    },

    // Popular routes
    popularRoutesTitle: "Popular Routes",
    routeTableHeaders: {
      route: "Route",
      distance: "Distance",
      duration: "Duration",
    },

    // CTA
    ctaTitle: "Ready to Book Your Transfer?",
    ctaSubtitle:
      "Get in touch for confirmation. Available 24/7 for airport pickups and drop-offs.",
  },

  // Services page
  servicesPage: {
    heroTitle: "Our Services",
    heroSubtitle:
      "From a quick airport pickup to a full day exploring Thessaloniki, we offer a range of professional transfer services tailored to your needs.",

    services: {
      airportPickup: {
        title: "Airport Pickup",
        description:
          "Start your trip in Thessaloniki with zero stress. I personally meet you at the arrivals gate with a name sign and help with your luggage.",
        highlight: "Most Popular",
        features: [
          "Meet & greet at arrivals hall",
          "Real-time flight tracking",
          "Free waiting time for flight delays",
          "Help with luggage",
          "Comfortable air-conditioned sedan",
        ],
      },
      airportDropoff: {
        title: "Airport Drop-off",
        description:
          "Get to Thessaloniki Airport on time, every time. I pick you up from any address in the city or surrounding area and drive you directly to your terminal.",
        features: [
          "Pickup from any address",
          "Timely departure to match your flight",
          "Luggage assistance",
          "Comfortable and clean vehicle",
          "Fixed price, no meter surprises",
        ],
      },
      cityTours: {
        title: "City Tours & Sightseeing",
        description:
          "Discover Thessaloniki like a local. I take you to the most iconic spots and hidden gems of this beautiful city, at your own pace.",
        features: [
          "White Tower & waterfront",
          "Ano Poli (Upper Town)",
          "Roman & Byzantine monuments",
          "Local food market stops",
          "Flexible duration and itinerary",
          "Local tips and recommendations",
        ],
      },
      halkidiki: {
        title: "Halkidiki Transfers",
        description:
          "Heading to the beaches of Halkidiki? I offer direct transfers from Thessaloniki Airport or city center to anywhere in Halkidiki.",
        highlight: "Popular in Summer",
        features: [
          "Direct airport to Halkidiki transfer",
          "Scenic coastal routes",
          "Spacious vehicle for beach gear",
          "Round-trip packages available",
        ],
      },
      longDistance: {
        title: "Long Distance Transfers",
        description:
          "Need to get further? I provide comfortable long-distance transfers to cities and destinations across northern Greece and beyond.",
        features: [
          "Kavala, Drama, Serres",
          "Katerini, Litochoro (Mount Olympus)",
          "Alexandroupoli",
          "Cross-border transfers available",
          "Rest stops as needed",
        ],
      },
      hotelEvent: {
        title: "Hotel & Event Transfers",
        description:
          "Attending a conference, or special event? I provide reliable transfers between any two locations in the Thessaloniki area.",
        features: [
          "Hotel to venue transfers",
          "Conference & event pickups",
          "Round-trip service",
          "On-time guarantee",
          "Professional and discreet",
        ],
      },
    },

    // How it works
    howItWorksTitle: "How It Works",
    steps: {
      step1: {
        title: "Get a Quote",
        description:
          "Contact us with your pickup and drop-off locations.",
      },
      step2: {
        title: "Confirm Booking",
        description:
          "We reply with the price and availability. Confirm your booking with just a message -- no deposits or apps needed.",
      },
      step3: {
        title: "Enjoy the Ride",
        description:
          "We pick you up on time, help with your luggage, and drive you safely to your destination.",
      },
    },
  },

  // Contact page
  contactPage: {
    heroTitle: "Get in Touch",
    heroSubtitle:
      "Ready to book your transfer or have a question? We are available 24/7 and happy to help.",

    // Contact form section
    sendMessage: "Send a Message",
    contactFormTitle: "Contact Form",
    contactFormDescription:
      "Have a specific question or need a custom quote? Fill out the form below and we will get back to you as soon as possible.",

    // Operating hours
    operatingHours: "Operating Hours",
    transfers: "Transfers",
    response: "Response",
    lessThan30Min: "< 30 min",
  },

  // Contact form
  contactForm: {
    fullName: "Full Name",
    namePlaceholder: "Your name",
    email: "Email",
    emailPlaceholder: "your@email.com",
    phone: "Phone (optional)",
    phonePlaceholder: "+30 XXX XXX XXXX",
    message: "Message",
    messagePlaceholder:
      "Tell us about your transfer needs (date, time, destination, number of passengers...)",
    sendMessage: "Send Message",

    // Success state
    messageSent: "Message Sent!",
    thankYou:
      "Thank you for reaching out. We will get back to you within a few hours. For faster response, reach us directly via WhatsApp.",
    sendAnother: "Send Another Message",
  },

  // Fare calculator page
  fareCalculatorPage: {
    heroTitle: "Fare Calculator",
    heroSubtitle:
      "Get an instant estimate for your transfer. Enter your pickup and drop-off locations to see the distance, duration, and fare.",

    // Pricing info
    pricingDetails: "Pricing Details",
    baseFare: "Base fare",
    ratePerKm: "Rate per km",
    minimumFare: "Minimum fare",

    // Disclaimer
    disclaimer:
      "Prices shown are estimates based on distance. Actual fare may vary slightly based on traffic conditions and specific pickup/drop-off points. Final price is confirmed upon booking.",

    // Popular routes sidebar
    popularRoutes: "Popular Routes",
  },

  // Fare calculator form
  fareForm: {
    title: "Calculate Your Fare",
    description:
      "Start typing and select your pickup and destination from the suggestions to get an instant price estimate.",
    pickupLocation: "Pickup Location",
    pickupPlaceholder: "e.g., Thessaloniki Airport",
    destination: "Destination",
    destinationPlaceholder: "e.g., Aristotelous Square, Thessaloniki",
    calculateFare: "Calculate Fare",
    calculating: "Calculating...",
    selectSuggestion: "Select a suggestion from the dropdown for accurate results",
    clear: "Clear",
    locationConfirmed: "Location confirmed",

    // Results
    yourFareEstimate: "Your Fare Estimate",
    fromLabel: "From",
    toLabel: "To",
    fare: "Fare",
    distance: "Distance",
    totalEstimate: "Total estimate",
    bookThisTrip: "Book This Trip via WhatsApp",
  },

  // Metadata
  meta: {
    homeTitle: "Ekos Transfers - Thessaloniki Airport Transfers",
    homeDescription:
      "Professional private driver service from Thessaloniki Airport (SKG) to any destination. Safe, comfortable, and always on time.",
    servicesTitle: "Services",
    servicesDescription:
      "Explore the services offered - airport pickups, drop-offs, city tours, and long-distance transfers in Thessaloniki, Greece.",
    contactTitle: "Contact",
    contactDescription:
      "Get in touch for airport transfers in Thessaloniki. Call, email, or WhatsApp us for instant booking.",
    fareCalculatorTitle: "Fare Calculator",
    fareCalculatorDescription:
      "Calculate your transfer fare from Thessaloniki Airport to any destination. Transparent pricing with no hidden costs.",
  },
} as const;

export type Translations = typeof en;
